...
first_in_pair = [4773, 4777, 4779, 4781, 4791, 4793, 4797, 4808, 4810, 4817,
                 4913, 4931, 4936, 4938, 6996, 7686, 7688, 7690, 7708, 9566, 10145, 
...
second_in_pair = [4774, 4778, 4780, 4782, 4792, 4794, 4798, 4809, 4811, 4818,
                  4914, 4932, 4937, 4939, 6981, 7687, 7689, 7691, 7709, 9563, 10146, 
...
def random_questions_random_sample(queryset, samplesize):
    ...
    if first_in_pair is None or second_in_pair is None:
        ...
        cache.add("Paired_questions_first_in_pair", json.dumps(first_in_pair), timeout=28800)
        cache.add("Paired_questions_second_in_pair", json.dumps(second_in_pair), timeout=None)
    else:
        first_in_pair = json.loads(first_in_pair)
        second_in_pair = json.loads(second_in_pair)
...
def get_random_home_folders(n=12):
    ''' Generates Random folders List'''
    avail_ids = list(Folder.objects.values_list('id', flat=True))
    ...
    while True:
        try:
            ...
            try:
                return get_random_home_folders(n=len(avail_ids))
            except ValueError:
                return []
        folder_return_list = []
        for pk in pkl:
            ...
            if folder:
                folder_return_list.append(folder)
            else:
                break
...
#     ''' Generates Random Question List based on folder'''
#     results = []
